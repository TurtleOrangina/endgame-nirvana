...
export default defineConfig({
  fmt: {
    semi: false,
    singleQuote: true,
    // The engine-playout measurement writes and reads these back itself
    // (src/measurements/engine-playout/yaml.ts): one flow mapping per line, JSON scalars.
    // Reformatting them across lines makes the puzzle set unreadable to its own parser.
    ignorePatterns: ['engine-playout-*.yaml'],
  },
  lint: {
  ...
  // of the app (the engine-playout measurement) opt back out with a
