export const en = {
  common: {
    email: 'Email',
    password: 'Password',
    passwordPlaceholder: 'At least 6 characters',
    cancel: 'Cancel',
    expand: 'Expand',
    collapse: 'Collapse',
    enterValidEmail: 'Enter a valid email address.',
    forgotPassword: 'Forgot password?',
    or: 'or',
    continueWithGoogle: 'Continue with Google',
    resetEmailSent:
      'A password reset email was sent to {email}. Click the link in it to choose a new password.',
  },
  app: {
    loadingExercises: 'Loading exercises…',
    analysisTitle: 'Analysis',
    youPlayWhite: 'You play white',
    youPlayBlack: 'You play black',
    versus: 'vs',
    surrender: 'Surrender',
    surrenderTitle: 'Give up this puzzle',
    retry: 'Retry',
    retryTitle: 'Retry this puzzle',
    next: 'Next',
    nextTitle: 'Next puzzle (N)',
    nextTitleGuest: 'Create a profile to keep training',
    guest: 'Guest',
    guestTitle: 'You are only viewing a shared puzzle — tap to create a profile',
    wrongSolution: 'Wrong solution!',
    analyse: 'Analyse',
    share: 'Share puzzle',
    shareTitle: 'Share a link to this puzzle',
    linkCopied: 'Link copied!',
    currentLevel: 'Current level:',
    session: 'Session:',
    solvedCount: '{count} solved',
    failedCount: '{count} failed',
    allCategories: 'All Categories',
    noPawnsSuffix: '(No pawns)',
    categoryChipTitle: 'Filter puzzles to this category',
    subcategoryChipTitle: 'Filter puzzles to this subcategory',
    themeChipTitle: 'A theme this puzzle illustrates',
    resultWin: 'win',
    resultDraw: 'draw',
    resultLoss: 'loss',
    progressSummary: '{solved} solved · {failed} failed · {left} left',
    noMatchingExercises: 'No exercises match the current filters.',
    unknownPuzzle: 'Unknown puzzle — the link may be outdated or the puzzle no longer exists.',
    unknownPuzzleNext: 'Pick a random puzzle',
    allCompletedInCategory: '🎉 You have completed all exercises in this category 🎉',
    categorySolveRate: 'Solve rate: {percent}%',
    noMoreAvailableInCategory: 'No more exercises available in this category right now.',
    hiddenTooHard: '{count} puzzles are hidden because they are too hard for your level.',
    allowSolvingTooHard: 'Allow solving of too hard puzzles',
    hiddenTooEasy: '{count} puzzles are hidden because they are too easy for your level.',
    allowSolvingTooEasy: 'Allow solving of too easy puzzles',
    engineDownloading: 'Downloading engine… {percent}% ({speed})',
    engineDownloadingWithEta: 'Downloading engine… {percent}% ({speed}, {eta} left)',
    engineLoading: 'Loading engine…',
    engineThinking: 'Engine is thinking…',
    navTraining: 'Training',
  },
  boardNav: {
    firstMove: 'First Move',
    previousMove: 'Previous Move (Left Arrow)',
    playBestMove: 'Play best move (Space)',
    nextMove: 'Next Move (Right Arrow)',
    lastMove: 'Last Move',
  },
  moveCounter: {
    movesSinceZeroTooltip:
      'Number of moves played since the last capture or pawn move. Draw if it reaches 50!',
  },
  board: {
    promotion: {
      queen: 'Queen',
      rook: 'Rook',
      knight: 'Knight',
      bishop: 'Bishop',
    },
    checkmate: 'Checkmate!',
    draw: 'Draw',
    stalemate: 'Stalemate',
    threefoldRepetition: 'Threefold repetition',
    fiftyMoveRule: 'Fifty-move rule',
    insufficientMaterial: 'Insufficient material',
  },
  analysis: {
    pauseEngine: 'Pause engine',
    resumeEngine: 'Resume engine',
    headerEval: 'Eval: {score} Depth {depth}',
    calculating: 'Calculating…',
    idle: 'Idle',
    settings: 'Settings',
    numLines: 'Number of lines',
    searchTime: 'Search time',
    cpuThreads: 'CPU threads',
    showBestArrow: 'Show best engine move arrow',
    showTablebaseArrow: 'Show best tablebase move arrow',
    engineThinkingHint: 'Engine thinking…',
    tablebase: 'Tablebase',
    chipCheckmate: 'checkmate',
    chipStalemate: 'stalemate',
    chipInsufficientMaterial: 'insufficient material',
    chipCapture: 'capture',
    chipPawnMove: 'pawn move',
    chipDraw: 'draw',
    categories: {
      win: 'win',
      'syzygy-win': 'syzygy-win',
      'maybe-win': 'maybe-win',
      'cursed-win': 'cursed-win',
      draw: 'draw',
      'blessed-loss': 'blessed-loss',
      'maybe-loss': 'maybe-loss',
      'syzygy-loss': 'syzygy-loss',
      loss: 'loss',
      unknown: 'unknown',
    },
    showLess: 'Show less',
    showMore: 'Show {count} more',
    leaveAnalysis: 'Leave Analysis Mode',
    fen: 'FEN',
    copyFen: 'Copy FEN',
  },
  setup: {
    welcomeBack: 'Welcome back!',
    welcomeTitle: 'Welcome to Endgame Nirvana',
    accountStepTitle: 'Set up your account',
    confirmEmailTitle: 'Confirm your email',
    lichessStepTitle: 'Link your Lichess account',
    signinSubtitle: 'Login to continue your endgame improvement',
    newSubtitle: 'The place to practice your endgames — where endgames become second nature.',
    newUserTab: 'New User',
    signInTab: 'Sign in',
    emailPlaceholder: 'you@example.com',
    signingIn: 'Signing in…',
    signInButton: 'Sign In',
    basicsInfo:
      'Pick a username and a starting level that is roughly realistic for your (endgame) Elo. Exercises are picked to match your level, and your level will adapt as you progress through them.',
    nameLabel: 'Name',
    nicknamePlaceholder: 'Your nickname',
    startingLevel: 'Starting level',
    levelBeginner: 'Beginner',
    levelBeginnerHint: 'You know how the pieces move.',
    levelNovice: 'Novice',
    levelNoviceHint: 'You can checkmate with king and rook vs king.',
    levelIntermediate: 'Intermediate',
    levelIntermediateHint: 'You know the rule of the square and the opposition.',
    levelClubPlayer: 'Club player',
    levelClubPlayerHint: 'You can checkmate with bishop and knight.',
    levelMaster: 'Master',
    levelMasterHint: 'You can checkmate with rook and bishop vs rook from the Philidor position.',
    levelGrandmaster: 'Grandmaster',
    levelGrandmasterHint: 'You can defend rook vs rook and bishop on the edge of the board.',
    continue: 'Continue',
    accountInfo:
      "Creating an account is optional. It lets you keep your progress in sync when you switch devices or browsers — without one, your progress is only stored in this browser and could be lost if it's cleared.",
    creating: 'Creating…',
    createAccount: 'Create account',
    continueWithoutAccount: 'Continue without account',
    back: '‹ Back',
    confirmationIntro: "We've sent a confirmation email to",
    confirmationOutro:
      '. Once you have clicked the activation link you can log in and your progress will be stored online. You can also train locally, but your progress will not be stored until you activate your account.',
    notActivatedYet: 'Not activated yet — click the link in the confirmation email first.',
    checking: 'Checking…',
    logInNow: 'Log In Now',
    trainLocally: 'Train Locally',
    lichessInfo:
      "Linking your Lichess account unlocks Endgame Tablebases. They're shown in analysis, but more importantly they let the computer opponent choose moves that are more varied and testing — better training than without.",
    linkLichess: 'Link Lichess',
    lichessLinkedTitle: 'Lichess account linked',
    lichessLinkedIntro: 'Your Lichess account',
    lichessLinkedOutro: ' was successfully linked.',
    next: 'Next',
    continueWithoutLichess: 'Continue without Lichess account',
    errorBothOrNeither: 'Enter both an email and password, or leave both blank to train locally.',
    errorSignupFailed:
      'Could not create your account. You can retry, or continue without an account and retry later from your profile page.',
    errorInvalidCredentials:
      'Wrong email or password. If you have forgotten your password, you can reset it via email.',
    errorServerUnreachable:
      'Could not reach the server. Please check your connection and try again.',
    errorServerUnreachableOfflineHint: 'Or create a',
    errorServerUnreachableOfflineLink: 'New User',
    errorServerUnreachableOfflineOutro: 'without account to continue offline.',
    emailAlreadyRegistered: 'This email is already registered.',
    logInInstead: 'Log in instead',
    sendResetEmail: 'Reset password via email',
    setupCompleteTitle: 'Setup completed!',
    startTraining: 'Start Training',
  },
  about: {
    navTitle: 'About',
    title: 'What is Endgame Nirvana?',
    intro:
      'Endgame Nirvana is about practicing endgame positions against a computer. Practice makes perfect, and actually converting the endgames and learning the intricacies is an absolute necessity. You do not need to find the absolute best (only) moves like in typical tactics puzzles. In Endgame Nirvana there are often multiple moves that work — you continue to play the position as if it was a normal chess game. It only ends once you convert the position correctly (e.g. checkmate for won positions, or holding the draw for drawn positions).',
    moveSelectionTitle: 'Move Selection',
    moveSelectionIntro:
      'Move selection by the computer was programmed to be challenging and varied — not simply the "best engine move" is played:',
    moveSelectionDefense:
      "The computer tries to defend in ways that make it harder for you to convert, just like a human would. For example, if you have to defend a drawn king vs king and pawn position, a typical engine will just sacrifice the pawn, because the resulting position is also drawn. Endgame Nirvana will test your king's dancing shoes and make sure you know how to keep that pawn at bay, letting you really learn how to defend against tenacious attacks.",
    moveSelectionVariety:
      'The computer varies its moves, picking among several viable defensive variants, so you learn all the different aspects of an endgame. E.g. when practicing a king, bishop and knight vs king endgame, there might be one move for the computer that leads to mate in 14 moves, and another move that leads to mate in 12 moves. The computer will pick randomly between these moves (with a higher emphasis on the longer-lasting mate in 14) — this way you will also learn the alternative "mate in 12" variation when repeating a few times. This gives a much more complete practice: by repeating an endgame a few times you develop a complete picture and really master it in a way that will hold up against human opponents (who will not only play the absolute longest-lasting defensive moves).',
    openTitle: 'Open and Free',
    openText:
      'Endgame Nirvana is open source and completely free. There are no advertisements, no premium version and no hidden financial objectives.',
    offlineTitle: 'Fully usable offline',
    offlineIntro:
      "No internet, no problem. As long as you have opened the page once (or installed the progressive web app), you can practice your endgames — you don't even need to create an account at all. The only functionality that depends on internet access:",
    offlineTablebase:
      'The Lichess-powered tablebase look-up is queried online. Without internet you will not see tablebase results.',
    offlineSync:
      'If you created an account, your solve progress and Elo will only be synced once you have internet access.',
  },
  profile: {
    retrying: 'Retrying…',
    retry: 'Retry',
    creatingAccount: 'Creating account…',
    createAccount: 'Create Account',
    confirmationIntro: 'We sent a confirmation email to',
    confirmationOutro: '. Click the activation link in that email, then retry signing in below.',
    signingIn: 'Signing in…',
    retrySignIn: 'Retry Sign In',
    localOnlyNotice: 'You have not created an account, your progress is only stored locally!',
    accountNotCreatedNotice:
      'Your account could not be created yet, your progress is only stored locally!',
    createAccountTitle: 'Create Account',
    pendingIntro: 'Account creation for',
    pendingOutro: "hasn't completed yet — enter your password to retry.",
    emailAlreadyRegistered:
      'This email is already registered. Use "Forgot password?" below to reset it via email and sign in.',
    solveProgress: 'Solve Progress',
    eloTitle: 'Elo',
    currentEloLabel: 'Current',
    last7Days: 'Last 7 days',
    last30Days: 'Last 30 days',
    categoryProgressTitle: 'Completion Progress by Category',
    totalProgress: 'Total',
    progressTooltip: {
      solved: 'Solved',
      failed: 'Failed',
      remaining: 'Remaining',
      hiddenForLevel: 'Hidden for your level',
      solveRate: 'Solve rate',
      completionRate: 'Completion',
      percentValue: '{percent}%',
    },
    noExercisesAttempted: 'No exercises attempted yet.',
    puzzleHistory: 'Puzzle History',
    noPuzzlesAttempted: 'No puzzles attempted yet.',
    replayPuzzleTitle: 'Click to replay this puzzle',
    browseExercises: 'Browse Exercises',
    settingsTitle: 'Settings',
    searchCategoriesPlaceholder: 'Search categories…',
    noCategoriesMatchSearch: 'No categories match your search.',
    noCategoriesMatchFilters: 'No categories match your filters.',
    browseFilterCompletionLabel: 'Completion',
    browseFilterDifficultyLabel: 'Difficulty',
    browseFilterShowAll: 'Show all',
    browseFilterOnlyCompleted: 'Only completed',
    browseFilterOnlySolved: 'Only solved',
    browseFilterOnlyFailed: 'Only failed',
    browseFilterOnlyNotCompleted: 'Only not completed',
    browseFilterOnlyAroundMyLevel: 'Only around my level',
    browseFilterOnlyBelowMyLevel: 'Only below my level',
    browseFilterOnlyAboveMyLevel: 'Only above my level',
    puzzleDifficulty: 'Training Settings',
    puzzleDifficultyLabel: 'Puzzle difficulties',
    difficultyQuestion: 'Which puzzles should count towards your training pool?',
    difficultyAround: 'Puzzles around my level',
    difficultyAroundAndAbove: 'Puzzles around and above my level',
    difficultyAroundAndBelow: 'Puzzles below and around my level',
    difficultyAll: 'All puzzle difficulties',
    activePuzzleCount: '{count} puzzles are active!',
    inactiveDetail: 'Inactive: {detail}',
    tooHardCount: '{count} too hard',
    tooEasyCount: '{count} too easy',
    lichessTitle: 'Lichess Account',
    linkedAs: 'Linked as',
    lichessNotLinked: 'Not linked',
    lichessDescription:
      'Link your Lichess account to enable tablebase analysis and better computer move suggestions.',
    linkLichessAccount: 'Link Lichess Account',
    unlinkLichessAccount: 'Unlink Lichess',
    colorSchemeTitle: 'Colour Scheme',
    mode: {
      title: 'Appearance',
      description: "Choose the app's colour scheme.",
      dark: 'Dark',
      light: 'Light',
      system: 'System',
    },
    appearance: {
      boardTitle: 'Board',
      pieceSetTitle: 'Pieces',
    },
    language: {
      title: 'Language',
      description: "Choose the app's language.",
    },
    engine: {
      title: 'Engine',
      threadsLabel: 'CPU threads',
      threadsHint: 'More threads improve move selection quality, but use more of your device.',
    },
    accountTitle: 'Account',
    endgameNirvanaAccountLabel: 'Endgame Nirvana Account',
    signedInAs: 'Signed in as',
    trainingLocallyOnly: 'Training locally on this device only.',
    noAccount: 'None',
    signedInWithGoogle: 'Signed in with Google',
    logOut: 'Log Out',
    deleteAccount: 'Delete Account',
    deleteProgress: 'Delete Progress',
    syncError: 'Sync error: {error}',
    syncing: 'Syncing…',
    unsyncedOne: '(1 unsynced change)',
    unsyncedMany: '({count} unsynced changes)',
  },
  deleteAccountModal: {
    title: 'Delete account?',
    body: 'You will lose all your progress and settings. This action cannot be undone.',
    confirm: 'Delete Account',
  },
  emailConfirmation: {
    confirmedTitle: 'Email confirmed',
    confirmedMessage: 'Your email address is confirmed and you are now signed in.',
    invalidTitle: 'Confirmation failed',
    linkInvalid: 'This confirmation link is invalid or has expired.',
    ok: 'OK',
  },
  passwordRecovery: {
    title: 'Choose a new password',
    subtitle: 'Enter a new password for your account.',
    newPasswordLabel: 'New password',
    linkInvalid: 'This password reset link is invalid or has expired. Please request a new one.',
    saving: 'Saving…',
    savePassword: 'Save Password',
  },
  // Display names for the category segments of public/exercises.json — see
  // src/utils/categoryLabels.ts, which maps the raw segments onto these keys. `search` holds
  // the plain words the category list can be searched by, since a name written in figurine
  // notation ("♖ vs ♝") matches nothing a user would type.
  exerciseCategories: {
    purePieces: { name: 'Pure Pieces Endgames', search: 'pure pieces basic pawnless' },
    pawns: { name: 'Pawn Endgames', search: 'pawn endgames' },
    knightVsPawns: { name: '♘ vs Pawns', search: 'knight vs pawns' },
    knightVsKnight: { name: '♘ vs ♞', search: 'knight vs knight' },
    bishopVsPawns: { name: '♗ vs Pawns', search: 'bishop vs pawns' },
    bishopVsBishop: { name: '♗ vs ♝', search: 'bishop vs bishop' },
    oppositeColour: { name: 'Opposite Colour', search: 'opposite colour colored' },
    sameColour: { name: 'Same Colour', search: 'same colour colored' },
    bishopVsKnight: { name: '♗ vs ♞', search: 'bishop vs knight' },
    rookVsPawns: { name: '♖ vs Pawns', search: 'rook vs pawns' },
    rookVsRook: { name: '♖ vs ♜', search: 'rook vs rook' },
    rookVsKnight: { name: '♖ vs ♞', search: 'rook vs knight' },
    rookVsBishop: { name: '♖ vs ♝', search: 'rook vs bishop' },
    queenVsQueen: { name: '♕ vs ♛', search: 'queen vs queen' },
    queenVsRook: { name: '♕ vs ♜', search: 'queen vs rook' },
    bishopKnightMate: { name: '♔♗♘ vs ♚', search: 'bishop knight vs king mate' },
    queenVsQueenOnly: { name: '♔♕ vs ♚♛', search: 'queen vs queen' },
    queenVsRookOnly: { name: '♔♕ vs ♚♜', search: 'queen vs rook' },
    rookVsBishopOnly: { name: '♔♖ vs ♚♝', search: 'rook vs bishop' },
    rookVsKnightOnly: { name: '♔♖ vs ♚♞', search: 'rook vs knight' },
    rookBishopVsRook: { name: '♔♖♗ vs ♚♜', search: 'rook bishop vs rook philidor' },
  },
  // Display names for the theme tags in public/exercises.json — see src/utils/puzzleTags.ts.
  exerciseTags: {
    breakthrough: 'Breakthrough',
    connectedPawns: 'Connected Pawns',
    correspondingSquares: 'Corresponding Squares',
    floatingSquare: 'Floating Square',
    minedSquares: 'Mined Squares',
    opposition: 'Opposition',
    outflanking: 'Outflanking',
    outsidePassedPawn: 'Outside Passed Pawn',
    pawnRace: 'Pawn Race',
    pendulumZigzag: 'Pendulum/Zigzag',
    protectedPassedPawn: 'Protected Passed Pawn',
    queenVsPawnOn7th: 'Queen vs Pawn on 7th',
    reserveTempo: 'Reserve Tempo',
    rookVsConnectedPawns: 'Rook vs Connected Pawns',
    rookVsSeparatedPawns: 'Rook vs Separated Pawns',
    selfProtectingPawns: 'Self-Protecting Pawns',
    separatedPawns: 'Separated Pawns',
    shouldering: 'Shouldering',
    squareOfThePawn: 'Square of the Pawn',
    stalemate: 'Stalemate',
    triangulation: 'Triangulation',
    undermining: 'Undermining',
  },
}
