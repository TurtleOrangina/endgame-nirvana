...
- `attempts` — synced completion history; `(user_id, client_attempt_id)` is the primary key (no surrogate id), so replaying a sync batch is a no-op. Own-row select only; all writes go through `record_attempts`. `user_elo_before` is an RPC input (needed for the puzzle-Elo math) but not a column — it's always `new_elo - elo_change`.

**RPCs** (`security definer`, `set search_path = ''`, execute revoked from `anon`):
- `record_attempts(p_attempts jsonb) → void` — batched idempotent attempt sync. For each newly-inserted attempt, atomically updates the matching puzzle's Elo (puzzle is server-authoritative; user Elo is client-authoritative and just gets persisted as-is) and increments the caller's profile `puzzles_solved`/`puzzles_failed` counters (server-derived, exactly-once thanks to the dedupe). Trims each user's attempts older than 8 weeks. Granted to `authenticated`.
