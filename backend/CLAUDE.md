...
- `initialize_oauth_profile(p_username text, p_start_elo integer) → void` — the OAuth counterpart to the signup metadata `handle_new_user` reads. Deliberately one-shot: it only ever fills a *null* `username`, so it can't be replayed to reset a played-in Elo. Granted to `authenticated`.
...
- **Google login** (`GOOGLE_ENABLED`/`GOOGLE_CLIENT_ID`/`GOOGLE_SECRET`) — commented out in `.env.example`; the matching `GOTRUE_EXTERNAL_GOOGLE_*` lines in `docker-compose.yml` must be uncommented too. Production doesn't use any of this: the provider is configured in the Supabase Cloud dashboard, with `https://<project-ref>.supabase.co/auth/v1/callback` as the authorized redirect URI in the Google Cloud console.

After editing any of these, run **`sh run.sh start`** (`docker compose up -d --wait`), which recreates the containers whose config changed. Do *not* use `sh run.sh restart auth` for an `.env` change: `docker compose restart` restarts the existing container with the environment it was created with, so the new values silently never reach GoTrue — which then still reports e.g. `Unsupported provider: provider is not enabled` from `/auth/v1/authorize`. Verify with `docker compose exec auth env | grep GOTRUE_`.

## Google's surplus profile claims are stripped

GoTrue persists a provider's entire userinfo payload — into `auth.users.raw_user_meta_data` and `auth.identities.identity_data`, **rewritten on every sign-in**, not just the first. Google's includes the user's real name, avatar URL and locale, none of which this app uses, and storing them is a materially different privacy posture than storing a self-chosen nickname (the Datenschutz page in `frontend/src/pages/LegalPage.vue` is written to match). Two `before insert or update` triggers in `20260728120000_google_oauth.sql` prune both columns down to an **allowlist** — `sub`/`email`/`email_verified`/`phone_verified` plus this app's own `username`/`start_elo` — so a provider adding a new claim later is discarded by default rather than silently stored. (`phone_verified` is GoTrue's own bookkeeping, not a provider claim, and holds no personal data; it's kept because the auth server reads it back in its own flows.) The subject id itself can't be dropped: `auth.identities.provider_id` *is* the sub, and it's how GoTrue matches a returning login back to an existing user.
