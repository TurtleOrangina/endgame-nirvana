...
# Shell plugins that auto-source .env (e.g. zsh dotenv) export copies of its
# variables with shell quoting applied, mangling JSON values like JWT_KEYS.
# Docker compose prefers the shell environment over .env, so unset every
# variable named in .env to make compose read the file's values.
if [ -f .env ]; then
    # shellcheck disable=SC2046
    for env_var in $(grep -o '^[A-Za-z_][A-Za-z0-9_]*' .env); do
